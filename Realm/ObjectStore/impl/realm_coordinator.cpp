    ...
    if (new_queries.empty()) {
        ...
        for (auto& query : new_queries) {
            if (version != query->version()) {
                transaction::advance_and_observe_linkviews(*m_advancer_sg, *m_advancer_history, *info, query->version());
        ...
        transaction::advance_and_observe_linkviews(*m_advancer_sg, *m_advancer_history, *info);
